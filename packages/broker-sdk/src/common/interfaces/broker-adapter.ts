...
export interface BrokerAdapterEventData {
  authenticated: { brokerAccountId: string; accountId: string };
  unauthenticated: { reason: string };
  order_placed: { orderId: string; symbol: string };
  order_filled: { orderId: string; fillPrice: number; filledQuantity: number };
  order_rejected: { orderId: string; reason: string };
  order_cancelled: { orderId: string };
  position_updated: { symbol: string; quantity: number };
  funds_updated: { availableCash: number; usedMargin: number };
...
export interface BrokerAdapter {
   ...
   */
  getTrades(filters?: { symbol?: string; from?: number; to?: number }): Promise<BrokerTrade[]>;
