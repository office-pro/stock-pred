...
if [ ! -f .env.test-cloud ]; then
  ...
  echo "Create it with Supabase DATABASE_URL and Upstash REDIS_URL (see docs/hybrid-dev-cloud.md)." >&2
  exit 1
fi

# Local defaults, then cloud overlay (cloud wins for DATABASE_URL / REDIS_URL).
...
if [ -z "${DATABASE_URL:-}" ] || [ -z "${REDIS_URL:-}" ]; then
  echo "ERROR: .env.test-cloud must set both DATABASE_URL and REDIS_URL." >&2
  exit 1
fi
...
# Refuse accidental local docker hostnames inside the cloud profile (host only).
case "${db_host}" in
...
case "${db_host}" in
  db.*.supabase.co)
    ...
    echo "In Supabase: Connect → Session pooler (port 5432)." >&2
...
case "${REDIS_URL}" in
  *redis:6379*|*localhost:6379*|redis://redis*|redis://localhost*|rediss://localhost*)
    echo "ERROR: REDIS_URL in .env.test-cloud still looks local. Use your Upstash rediss:// URI." >&2
    exit 1
    ;;
esac
...
if [ "${STOCK_UNIVERSE_MODE:-}" = "quick-start" ] && [ "${ALLOW_CLOUD_QUICK_START:-0}" != "1" ]; then
  echo "==> Overriding STOCK_UNIVERSE_MODE=quick-start → full-universe for start:all-cloud"
...
# Do not use local Postgres/Redis for this profile.
echo "==> Stopping local postgres/redis (if running) — cloud URLs are used instead"
...
# Supabase Session pooler free tier ≈ 15 clients. Running apps hold those slots
...
# Build equity-master.json before seed so full-universe migrate can load all NSE/BSE names.
# DB upsert may no-op if schema is not migrated yet; JSON snapshot is what seed needs.
if [ "${SKIP_LISTINGS_INGEST:-0}" != "1" ]; then
...
# Full membership ingest while apps are still paused (Session pooler ≈15 clients).
# NSE_ALL upserts every equity; also US / forex / crypto / futures gates.
...
# Belts and braces: local infra must stay down on this profile.
"${COMPOSE[@]}" stop postgres redis >/dev/null 2>&1 || true
docker compose stop postgres redis >/dev/null 2>&1 || true
